...
既存 PHP `app/domain.php` の中核を `src/lib/progress.ts` に移植しています。
合格(OK)で返却すると、割当の進度を1つ前進させ、**次の範囲の提出物(未提出)を自動生成**します。

- トラック種別: `chapter`(章ごと) / `number`(番号ごと) / `etore` / `manual`(手入力)
...
未対応: 教材ミックス(2教材交互) と eトレのバッチ列連携 (将来拡張)。
