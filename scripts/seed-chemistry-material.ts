...
const SET_LABELS = ["A-1", "A-2", "A-3", "B-1", "B-2", "B-3", "C-1", "C-2", "C-3"];
const EXPECTED_UNIT_COUNT = EXPECTED_TOPICS.length * SET_LABELS.length;
...
function buildUnitSources(chunks: PdfChunk[]): UnitSource[] {
  ...
  const missingTopics = EXPECTED_TOPICS.filter((topic) => !rows.some((r) => r.topic === topic));
  if (missingTopics.length > 0) throw new Error(`PDFから分野を検出できません: ${missingTopics.join(", ")}`);
  if (rows.length !== EXPECTED_UNIT_COUNT) {
    throw new Error(`検出した範囲数が想定外です: ${rows.length}件 (想定 ${EXPECTED_UNIT_COUNT}件)`);
  }
  return rows;
}
...
async function upsertMaterial(organizationId: string) {
  const description =
  ...
  let [m] = await db
    .select()
    .from(materials)
    .where(and(eq(materials.organizationId, organizationId), eq(materials.name, NAME)))
    .limit(1);
...
async function replaceFiles(
  ...
  for (let i = 0; i < createdUnits.length; i++) {
    ...
    for (const [kind, chunk] of [
      ...
      const pathname = `${organizationId}/materials/${materialId}/chem-${String(i + 1).padStart(3, "0")}-${kind}.pdf`;
...
async function main() {
  ...
  const srcBytes = readFileSync(PDF);
  const srcDoc = await PDFDocument.load(srcBytes);
  const chunks = await parseChunks(PDF);
  const unitSources = buildUnitSources(chunks);
  const org = await resolveOrg();

  console.log(`組織: ${org.name} (${org.id})${DRY ? "  [DRY RUN]" : ""}`);
  console.log(`PDF : ${PDF} (${(srcBytes.length / 1024 / 1024).toFixed(1)}MB / ${srcDoc.getPageCount()} pages)`);
  console.log(`教材: [${SUBJECT}] ${NAME} / division=secondary / progress=chapter`);
  ...
  const material = await upsertMaterial(org.id);
  const createdUnits = await replaceUnits(org.id, material.id, unitSources);
  await replaceFiles(org.id, material.id, srcDoc, createdUnits);
  ...
  console.log(`✓ 化学教材 登録完了: ${unitRows.length}範囲 / 問題+解答PDF / 既存空割当の補正 ${repaired}件`);
  process.exit(0);
}
